...
### [1] CONFIG · 2026-08-24 11:46:18 UTC

```
model: claude-haiku-4-5 | effort: low | permission: bypassPermissions | cwd: /ai-inventor/aii_data/runs/run_a-GYNIuwcKDN/3_invention_loop/iter_2/upd_hypo/upd_hypo
```

### [2] SYSTEM-USER prompt · 2026-08-24 11:46:24 UTC
...
### [3] HUMAN-USER prompt · 2026-08-24 11:46:24 UTC

```
Find a better heuristic for scheduling background jobs on a machine with bursty CPU load.
```

### [4] ASSISTANT · 2026-08-24 11:47:38 UTC

```
Looking at the evidence from this iteration, I need to make an honest assessment of what succeeded and what failed.

## Analysis

**What Succeeded:**
- 23.5% CV reduction (target ≥20%) ✓
- 11.4% p95 latency improvement (target ≥10%) ✓
- Both entropy and momentum components show large effect sizes
- Parameters reasonably robust (stability score 0.906)

**What Failed:**
- Entropy-outcome correlation r≈-0.09 vs. r>0.7 target — **MAJOR MISS**
- No real workload validation (synthetic-only, despite claiming both)
- No baseline comparisons to published methods
- Small sample size (n=10, below methodological rigor standards)

**Critical Gaps:**
- Artifact 4 documents that real workloads have entropy 0.4-1.0 bits vs. synthetic 0.2-0.6 bits — suggests parameters will need recalibration
- Weak entropy-outcome correlation undermines theoretical justification; mechanism unclear
- Production readiness claims are premature without real workload testing

## Revision Strategy

The core empirical finding IS valid: entropy+momentum adaptive scheduling works on synthetic bursty workloads. However, I must:

1. **Acknowledge the r>0.7 failure explicitly** and reframe from "entropy is optimal" to "empirical adaptation works despite weak direct signal"
2. **Narrow scope to synthetic validation only** — real workloads are flagged as critical gap
3. **Add mandatory real-world validation** (Alibaba 2018 traces) before production claims
4. **Increase sample size to n≥50** for statistical rigor
5. **Add baseline comparisons** as required, not optional
6. **Simplify parameter justification** — candidate values lack empirical backing beyond sensitivity range
7. **Downgrade confidence** due to unfulfilled targets and missing validation

This is an **evolution**: same core concept, but narrowed scope, acknowledged failure of a key target, and shifted from theoretical to pragmatic framing.
```

### [5] TOOL CALL — Write · 2026-08-24 11:47:38 UTC

```
File: /ai-inventor/aii_data/runs/run_a-GYNIuwcKDN/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json

{
  "title": "Entropy+Momentum Adaptive Scheduling (Synthetic Validation)",
  "hypothesis": "Background job scheduling improves under bursty workloads when thresholds adapt dynamically based on CPU load entropy (unpredictability) and momentum (rate of change), rather than using fixed absolute thresholds. Empirical validation on synthetic workloads achieves: (1) background job completion time variance reduced by ≥20% vs. fixed-55% threshold (measured 23.5%, p<0.001), (2) foreground p95 latency improved by ≥10% (measured 11.4%, p<0.001), (3) both entropy and momentum components demonstrably contribute (Cohen's d > 1.0 for each), (4) design parameters near-optimal within tested ranges, (5) superiority confirmed vs. momentum-only and entropy-only baselines. CRITICAL LIMITATION: Validation currently limited to synthetic Poisson workloads with periodic spikes; real production workloads exhibit different entropy distributions (0.4-1.0 bits vs. 0.2-0.6 bits synthetic) and temporal structure (multi-scale burstiness, diurnal patterns), requiring empirical re-validation. Entropy-outcome correlation is weak (r≈-0.09, target r>0.7 not achieved), suggesting entropy works empirically but mechanism is not direct signal correlation; alternative explanations (adaptive-thresholds-in-general, non-linear relationships) require investigation.",
  "motivation": "Current fixed CPU threshold schedulers fail under bursty loads because they cannot distinguish safe calm periods from temporary lulls before spikes. Entropy quantifies load unpredictability and momentum captures trends, enabling dynamic dispatch decisions. However, the weak entropy-outcome correlation (r≈-0.09) indicates entropy's theoretical optimality is unconfirmed; the contribution is empirical robustness of adaptive thresholds, not theoretically optimal signal selection.",
  "assumptions": [
    "CPU load entropy from 60s windows provides useful scheduling signal, even if not strongly correlated with instantaneous outcomes.",
    "Load momentum is useful for dispatch timing refinement.",
    "Background jobs tolerate adaptive scheduling windows.",
    "Entropy-based adaptation reduces variance vs. fixed thresholds on synthetic bursty workloads.",
    "Real production workloads require entropy re-calibration due to different distributions than synthetic models."
  ],
  "investigation_approach": "Expand and validate synthetic work: (1) Enlarge to n≥50 trials per scheduler to ensure statistical adequacy (vs. current n=10); report power analysis and stable confidence intervals. (2) Investigate entropy-outcome correlation failure: plot scatter entropy vs. latency delta; fit polynomial and non-linear models; test alternative signals (load variance, autocorrelation, ARIMA error); compare correlation strength across signals. (3) Add real workload validation: extract CPU load from Alibaba 2018 traces (primary dataset); run entropy-adaptive and baseline; measure CV and p95; compare entropy distributions via KS test; measure whether parameters need retuning. (4) Baseline comparisons: implement momentum-only (fixed 55% + momentum dispatch) and at least one published baseline (e.g., MoS workload-type classifier); report head-to-head comparison. (5) Empirically justify thresholds: grid-search momentum threshold, entropy safe-zone, and false-positive window on synthetic data; report sensitivity.",
  "success_criteria": "On synthetic workloads (n≥50): (a) ≥20% CV reduction vs. fixed-55% (p<0.05), (b) ≥10% p95 improvement (p<0.05), (c) entropy and momentum each Cohen's d>1.0 in ablation, (d) superiority over momentum-only and ≥1 published baseline on identical workload, (e) parameter choices justified via grid-search sensitivity analysis. Real workload validation (Alibaba 2018): (f) ≥15% CV reduction on real traces (lower bound accounting for entropy mismatch), (g) ≥8% p95 improvement on real traces, (h) entropy distribution divergence documented (KS test p-value and Cohen's d), (i) parameter recalibration strategy identified if needed.",
  "related_works": [
    "Mixture-of-Schedulers (2511.11628): Routes via workload-type ML classification. Differs: entropy-adaptive uses information theory for threshold adaptation, not workload classification; requires no training data.",
    "PREACT (3673038.3673135): QPS prediction at datacenter scale. Differs: entropy-adaptive operates single-machine, dispatch-time (not provision-time).",
    "DriftSched (2606.02982): GPU token drift adaptation. Differs: entropy-adaptive uses universally available CPU load, not GPU-specific signals.",
    "Load trend analysis: Uses forecasting for provisioning. Differs: entropy-adaptive uses current entropy+momentum for real-time dispatch decisions.",
    "RL entropy scheduling (2601.19624): Exploration policy tuning. Differs: context is learning, not CPU job scheduling."
  ],
  "terms": [
    {
      "term": "Load entropy",
      "definition": "Shannon entropy of recent CPU load samples (60s window, 8 bins), quantifying unpredictability. High entropy = volatile, risky to schedule; low entropy = smooth, safe to schedule."
    },
    {
      "term": "Momentum (load gradient)",
      "definition": "Instantaneous rate of change of CPU load (estimated via EWMA, α=0.2). Negative momentum = decreasing load (safe dispatch window); positive momentum = increasing load (risky dispatch)."
    },
    {
      "term": "Adaptive threshold",
      "definition": "CPU load limit that scales based on entropy (high entropy → conservative, low entropy → permissive), e.g., 70% - 45%×H_norm."
    },
    {
      "term": "Bursty workload",
      "definition": "High-variability request rate causing unpredictable CPU spikes even with moderate mean load."
    },
    {
      "term": "Background job",
      "definition": "Lower-priority task (GC, indexing, replication) tolerating variable scheduling delays without degrading foreground latency."
    }
  ],
  "summary": "Entropy-adaptive background job scheduling improves job completion time variance and foreground latency on synthetic bursty workloads by adjusting CPU thresholds based on entropy and momentum. Synthetic validation shows 23.5% variance reduction and 11.4% latency improvement. Scope is currently synthetic-only; real production workload validation (Alibaba 2018) is required before production claims. Weak entropy-outcome correlation (r≈-0.09) indicates mechanism is not theoretically grounded; alternative hypotheses and non-linear relationships require investigation.",
  "confidence_delta": "decreased",
  "key_changes": [
    "Acknowledged explicit failure of r>0.7 entropy-outcome correlation target (actual r≈−0.09); reframed from 'entropy is optimal' to 'empirical improvement despite weak signal'.",
    "Narrowed scope to synthetic-only validation; removed claim of validation 'on both synthetic and real workloads.'",
    "Added mandatory real-world validation requirement (Alibaba 2018 traces) with lower success bounds (≥15% CV, ≥8% p95) before production deployment claims.",
    "Increased sample size requirement from n=10 to n≥50 trials per scheduler for statistical rigor; added power analysis.",
    "Added mandatory baseline comparisons to published methods (MoS, PREACT, etc.) rather than optional future work.",
    "Emphasized mechanism uncertainty: weak entropy-outcome correlation suggests non-direct signal relationship; alternative hypotheses (adaptive-thresholds-in-general, non-linear relationships) flagged for investigation.",
    "Required empirical threshold justification (grid-search on synthetic data) rather than relying on candidate parameter values.",
    "Softened parameter robustness claims pending larger sensitivity analysis and real-world validation.",
    "Added entropy distribution divergence measurement (KS test) for synthetic vs. real comparison."
  ],
  "relation_type": "evolution",
  "relation_rationale": "Same core concept (entropy+momentum adaptive scheduling), but narrowed to synthetic-only validation, acknowledged r>0.7 target failure, and shifted from theoretical optimality to empirical pragmatism.",
  "artifact_relations": [
    {
      "from_id": "art_sTVyGK2NrtrJ",
      "to_id": "art_aWntWiTqo8og",
      "relation_type": "uses",
      "relation_rationale": "Experiment implements entropy-adaptive design spec (entropy estimator, momentum tracker, threshold formula)."
    }
  ]
}
```

### [6] TOOL RESULT — Write · 2026-08-24 11:47:38 UTC

```
{"type": "create", "filePath": "/ai-inventor/aii_data/runs/run_a-GYNIuwcKDN/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json", "content": "{\n  \"title\": \"Entropy+Momentum Adaptive Scheduling (Synthetic Validation)\",\n  \"hypothesis\": \"Background job scheduling improves under bursty workloads when thresholds adapt dynamically based on CPU load entropy (unpredictability) and momentum (rate of change), rather than using fixed absolute thresholds. Empirical validation on synthetic workloads achieves: (1) background job completion time variance reduced by ≥20% vs. fixed-55% threshold (measured 23.5%, p<0.001), (2) foreground p95 latency improved by ≥10% (measured 11.4%, p<0.001), (3) both entropy and momentum components demonstrably contribute (Cohen's d > 1.0 for each), (4) design parameters near-optimal within tested ranges, (5) superiority confirmed vs. momentum-only and entropy-only baselines. CRITICAL LIMITATION: Validation currently limited to synthetic Poisson workloads with periodic spikes; real production workloads exhibit different entropy distributions (0.4-1.0 bits vs. 0.2-0.6 bits synthetic) and temporal structure (multi-scale burstiness, diurnal patterns), requiring empirical re-validation. Entropy-outcome correlation is weak (r≈-0.09, target r>0.7 not achieved), suggesting entropy works empirically but mechanism is not direct signal correlation; alternative explanations (adaptive-thresholds-in-general, non-linear relationships) require investigation.\",\n  \"motivation\": \"Current fixed CPU threshold schedulers fail under bursty loads because they cannot distinguish safe calm periods from temporary lulls before spikes. Entropy quantifies load unpredictability and momentum captures trends, enabling dynamic dispatch decisions. However, the weak entropy-outcome correlation (r≈-0.09) indicates entropy's theoretical optimality is unconfirmed; the contribution is empirical robustness of adaptive thresholds, not theoretically optimal signal selection.\",\n  \"assumptions\": [\n    \"CPU load entropy from 60s windows provides useful scheduling signal, even if not strongly correlated with instantaneous outcomes.\",\n    \"Load momentum is useful for dispatch timing refinement.\",\n    \"Background jobs tolerate adaptive scheduling windows.\",\n    \"Entropy-based adaptation reduces variance vs. fixed thresholds on synthetic bursty workloads.\",\n    \"Real production workloads require entropy re-calibration due to different distributions than synthetic models.\"\n  ],\n  \"investigation_approach\": \"Expand and validate synthetic work: (1) Enlarge to n≥50 trials per scheduler to ensure statistical adequacy (vs. current n=10); report power analysis and stable confidence intervals. (2) Investigate entropy-outcome correlation failure: plot scatter entropy vs. latency delta; fit polynomial and non-linear models; test alternative signals (load variance, autocorrelation, ARIMA error); compare correlation strength across signals. (3) Add real workload validation: extract CPU load from Alibaba 2018 traces (primary dataset); run entropy-adaptive and baseline; measure CV and p95; compare entropy distributions via KS test; measure whether parameters need retuning. (4) Baseline comparisons: implement momentum-only (fixed 55% + momentum dispatch) and at least one published baseline (e.g., MoS workload-type classifier); report head-to-head comparison. (5) Empirically justify thresholds: grid-search momentum threshold, entropy safe-zone, and false-positive window on synthetic data; report sensitivity.\",\n  \"success_criteria\": \"On synthetic workloads (n≥50): (a) ≥20% CV reduction vs. fixed-55% (p<0.05), (b) ≥10% p95 improvement (p<0.05), (c) entropy and momentum each Cohen's d>1.0 in ablation, (d) superiority over momentum-only and ≥1 published baseline on identical workload, (e) parameter choices justified via grid-search sensitivity analysis. Real workload validation (Alibaba 2018): (f) ≥15% CV reduction on real traces (lower bound accounting for entropy mismatch), (g) ≥8% p95 improvement on real traces, (h) entropy distribution divergence documented (KS test p-value and Cohen's d), (i) parameter recalibration strategy identified if needed.\",\n  \"related_works\": [\n    \"Mixture-of-Schedulers (2511.11628): Routes via workload-type ML classification. Differs: entropy-adaptive uses information theory for threshold adaptation, not workload classification; requires no training data.\",\n    \"PREACT (3673038.3673135): QPS prediction at datacenter scale. Differs: entropy-adaptive operates single-machine, dispatch-time (not provision-time).\",\n    \"DriftSched (2606.02982): GPU token drift adaptation. Differs: entropy-adaptive uses universally available CPU load, not GPU-specific signals.\",\n    \"Load trend analysis: Uses forecasting for provisioning. Differs: entropy-adaptive uses current entropy+momentum for real-time dispatch decisions.\",\n    \"RL entropy scheduling (2601.19624): Exploration policy tuning. Differs: context is learning, not CPU job scheduling.\"\n  ],\n  \"terms\": [\n    {\n      \"term\": \"Load entropy\",\n      \"definition\": \"Shannon entropy of recent CPU load samples (60s window, 8 bins), quantifying unpredictability. High entropy = volatile, risky to schedule; low entropy = smooth, safe to schedule.\"\n    },\n    {\n      \"term\": \"Momentum (load gradient)\",\n      \"definition\": \"Instantaneous rate of change of CPU load (estimated via EWMA, α=0.2). Negative momentum = decreasing load (safe dispatch window); positive momentum = increasing load (risky dispatch).\"\n    },\n    {\n      \"term\": \"Adaptive threshold\",\n      \"definition\": \"CPU load limit that scales based on entropy (high entropy → conservative, low entropy → permissive), e.g., 70% - 45%×H_norm.\"\n    },\n    {\n      \"term\": \"Bursty workload\",\n      \"definition\": \"High-variability request rate causing unpredictable CPU spikes even with moderate mean load.\"\n    },\n    {\n      \"term\": \"Background job\",\n      \"definition\": \"Lower-priority task (GC, indexing, replication) tolerating variable scheduling delays without degrading foreground latency.\"\n    }\n  ],\n  \"summary\": \"Entropy-adaptive background job scheduling improves job completion time variance and foreground latency on synthetic bursty workloads by adjusting CPU thresholds based on entropy and momentum. Synthetic validation shows 23.5% variance reduction and 11.4% latency improvement. Scope is currently synthetic-only; real production workload validation (Alibaba 2018) is required before production claims. Weak entropy-outcome correlation (r≈-0.09) indicates mechanism is not theoretically grounded; alternative hypotheses and non-linear relationships require investigation.\",\n  \"confidence_delta\": \"decreased\",\n  \"key_changes\": [\n    \"Acknowledged explicit failure of r>0.7 entropy-outcome correlation target (actual r≈−0.09); reframed from 'entropy is optimal' to 'empirical improvement despite weak signal'.\",\n    \"Narrowed scope to synthetic-only validation; removed claim of validation 'on both synthetic and real workloads.'\",\n    \"Added mandatory real-world validation requirement (Alibaba 2018 traces) with lower success bounds (≥15% CV, ≥8% p95) before production deployment claims.\",\n    \"Increased sample size requirement from n=10 to n≥50 trials per scheduler for statistical rigor; added power analysis.\",\n    \"Added mandatory baseline comparisons to published methods (MoS, PREACT, etc.) rather than optional future work.\",\n    \"Emphasized mechanism uncertainty: weak entropy-outcome correlation suggests non-direct signal relationship; alternative hypotheses (adaptive-thresholds-in-general, non-linear relationships) flagged for investigation.\",\n    \"Required empirical threshold justification (grid-search on synthetic data) rather than relying on candidate parameter values.\",\n    \"Softened parameter robustness claims pending larger sensitivity analysis and real-world validation.\",\n    \"Added entropy distribution divergence measurement (KS test) for synthetic vs. real comparison.\"\n  ],\n  \"relation_type\": \"evolution\",\n  \"relation_rationale\": \"Same core concept (entropy+momentum adaptive scheduling), but narrowed to synthetic-only validation, acknowledged r>0.7 target failure, and shifted from theoretical optimality to empirical pragmatism.\",\n  \"artifact_relations\": [\n    {\n      \"from_id\": \"art_sTVyGK2NrtrJ\",\n      \"to_id\": \"art_aWntWiTqo8og\",\n      \"relation_type\": \"uses\",\n      \"relation_rationale\": \"Experiment implements entropy-adaptive design spec (entropy estimator, momentum tracker, threshold formula).\"\n    }\n  ]\n}\n", "structuredPatch": [], "originalFile": null, "userModified": false}
```

### [7] ASSISTANT · 2026-08-24 11:47:47 UTC

```
Done. I've revised the hypothesis based on the evidence from iteration 2.

## Summary of Revision

**What Changed:**

The core empirical contribution holds: entropy + momentum adaptive scheduling achieves 23.5% variance reduction and 11.4% p95 latency improvement on synthetic bursty workloads. However, I've made three critical shifts:

1. **Acknowledged the r≈-0.09 failure**: The hypothesis explicitly promised r>0.7 entropy-outcome correlation; it achieved r≈-0.09. This is not ignored or minimized — it's stated upfront as a "CRITICAL LIMITATION" that invalidates the theoretical claim that "entropy is THE optimal signal." The contribution reframes to empirical robustness despite weak direct signal correlation.

2. **Narrowed scope to synthetic-only**: Removed the claim of validation "on both synthetic and real workloads." Real production workloads have different entropy distributions (0.4-1.0 bits vs. 0.2-0.6 bits), different temporal structure, and require empirical re-validation before production deployment claims are credible.

3. **Made next steps mandatory, not optional**: Added explicit requirements for (a) n≥50 trials (vs. current n=10), (b) real workload testing on Alibaba 2018 traces, (c) baseline comparisons to published methods, (d) investigation of the entropy-outcome correlation failure via non-linear models and alternative signals.

**Confidence decreased** due to unfulfilled r>0.7 target and unvalidated real-world scope. This is an **evolution** (same core concept, refined scope and framing) rather than replacement or embedding.

The revised hypothesis is output to `./.terminal_claude_agent_struct_out.json`.
```

### [8] SYSTEM-USER prompt · 2026-08-24 11:47:48 UTC
...
### [9] TOOL CALL — Write · 2026-08-24 11:48:26 UTC
...

{
  ...
  "relation_type": "evolution",
  "relation_rationale": "Narrowed scope to synthetic; acknowledged r>0.7 failure; shifted to empirical basis.",
  "artifact_relations": [
    {
      "from_id": "art_sTVyGK2NrtrJ",
      "to_id": "art_aWntWiTqo8og",
...
### [10] TOOL RESULT — Write · 2026-08-24 11:48:26 UTC

```
{"type": "update", "filePath": "/ai-inventor/aii_data/runs/run_a-GYNIuwcKDN/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json", "content": "{\n  \"title\": \"Entropy+Momentum Adaptive Scheduling (Synthetic Validation)\",\n  \"hypothesis\": \"Background job scheduling improves under bursty workloads when thresholds adapt dynamically based on CPU load entropy (unpredictability) and momentum (rate of change), rather than using fixed absolute thresholds. Empirical validation on synthetic workloads achieves: (1) background job completion time variance reduced by ≥20% vs. fixed-55% threshold (measured 23.5%, p<0.001), (2) foreground p95 latency improved by ≥10% (measured 11.4%, p<0.001), (3) both entropy and momentum components demonstrably contribute (Cohen's d > 1.0 for each), (4) design parameters near-optimal within tested ranges, (5) superiority confirmed vs. momentum-only and entropy-only baselines. CRITICAL LIMITATION: Validation currently limited to synthetic Poisson workloads with periodic spikes; real production workloads exhibit different entropy distributions (0.4-1.0 bits vs. 0.2-0.6 bits synthetic) and temporal structure (multi-scale burstiness, diurnal patterns), requiring empirical re-validation. Entropy-outcome correlation is weak (r≈-0.09, target r>0.7 not achieved), suggesting entropy works empirically but mechanism is not direct signal correlation; alternative explanations (adaptive-thresholds-in-general, non-linear relationships) require investigation.\",\n  \"motivation\": \"Current fixed CPU threshold schedulers fail under bursty loads because they cannot distinguish safe calm periods from temporary lulls before spikes. Entropy quantifies load unpredictability and momentum captures trends, enabling dynamic dispatch decisions. However, the weak entropy-outcome correlation (r≈-0.09) indicates entropy's theoretical optimality is unconfirmed; the contribution is empirical robustness of adaptive thresholds, not theoretically optimal signal selection.\",\n  \"assumptions\": [\n    \"CPU load entropy from 60s windows provides useful scheduling signal, even if not strongly correlated with instantaneous outcomes.\",\n    \"Load momentum is useful for dispatch timing refinement.\",\n    \"Background jobs tolerate adaptive scheduling windows.\",\n    \"Entropy-based adaptation reduces variance vs. fixed thresholds on synthetic bursty workloads.\",\n    \"Real production workloads require entropy re-calibration due to different distributions than synthetic models.\"\n  ],\n  \"investigation_approach\": \"Expand and validate synthetic work: (1) Enlarge to n≥50 trials per scheduler to ensure statistical adequacy (vs. current n=10); report power analysis and stable confidence intervals. (2) Investigate entropy-outcome correlation failure: plot scatter entropy vs. latency delta; fit polynomial and non-linear models; test alternative signals (load variance, autocorrelation, ARIMA error); compare correlation strength across signals. (3) Add real workload validation: extract CPU load from Alibaba 2018 traces (primary dataset); run entropy-adaptive and baseline; measure CV and p95; compare entropy distributions via KS test; measure whether parameters need retuning. (4) Baseline comparisons: implement momentum-only (fixed 55% + momentum dispatch) and at least one published baseline (e.g., MoS workload-type classifier); report head-to-head comparison. (5) Empirically justify thresholds: grid-search momentum threshold, entropy safe-zone, and false-positive window on synthetic data; report sensitivity.\",\n  \"success_criteria\": \"On synthetic workloads (n≥50): (a) ≥20% CV reduction vs. fixed-55% (p<0.05), (b) ≥10% p95 improvement (p<0.05), (c) entropy and momentum each Cohen's d>1.0 in ablation, (d) superiority over momentum-only and ≥1 published baseline on identical workload, (e) parameter choices justified via grid-search sensitivity analysis. Real workload validation (Alibaba 2018): (f) ≥15% CV reduction on real traces (lower bound accounting for entropy mismatch), (g) ≥8% p95 improvement on real traces, (h) entropy distribution divergence documented (KS test p-value and Cohen's d), (i) parameter recalibration strategy identified if needed.\",\n  \"related_works\": [\n    \"Mixture-of-Schedulers (2511.11628): Routes via workload-type ML classification. Differs: entropy-adaptive uses information theory for threshold adaptation, not workload classification; requires no training data.\",\n    \"PREACT (3673038.3673135): QPS prediction at datacenter scale. Differs: entropy-adaptive operates single-machine, dispatch-time (not provision-time).\",\n    \"DriftSched (2606.02982): GPU token drift adaptation. Differs: entropy-adaptive uses universally available CPU load, not GPU-specific signals.\",\n    \"Load trend analysis: Uses forecasting for provisioning. Differs: entropy-adaptive uses current entropy+momentum for real-time dispatch decisions.\",\n    \"RL entropy scheduling (2601.19624): Exploration policy tuning. Differs: context is learning, not CPU job scheduling.\"\n  ],\n  \"terms\": [\n    {\n      \"term\": \"Load entropy\",\n      \"definition\": \"Shannon entropy of recent CPU load samples (60s window, 8 bins), quantifying unpredictability. High entropy = volatile, risky to schedule; low entropy = smooth, safe to schedule.\"\n    },\n    {\n      \"term\": \"Momentum (load gradient)\",\n      \"definition\": \"Instantaneous rate of change of CPU load (estimated via EWMA, α=0.2). Negative momentum = decreasing load (safe dispatch window); positive momentum = increasing load (risky dispatch).\"\n    },\n    {\n      \"term\": \"Adaptive threshold\",\n      \"definition\": \"CPU load limit that scales based on entropy (high entropy → conservative, low entropy → permissive), e.g., 70% - 45%×H_norm.\"\n    },\n    {\n      \"term\": \"Bursty workload\",\n      \"definition\": \"High-variability request rate causing unpredictable CPU spikes even with moderate mean load.\"\n    },\n    {\n      \"term\": \"Background job\",\n      \"definition\": \"Lower-priority task (GC, indexing, replication) tolerating variable scheduling delays without degrading foreground latency.\"\n    }\n  ],\n  \"summary\": \"Entropy-adaptive background job scheduling improves job completion time variance and foreground latency on synthetic bursty workloads by adjusting CPU thresholds based on entropy and momentum. Synthetic validation shows 23.5% variance reduction and 11.4% latency improvement. Scope is currently synthetic-only; real production workload validation (Alibaba 2018) is required before production claims. Weak entropy-outcome correlation (r≈-0.09) indicates mechanism is not theoretically grounded; alternative hypotheses and non-linear relationships require investigation.\",\n  \"confidence_delta\": \"decreased\",\n  \"key_changes\": [\n    \"Acknowledged explicit failure of r>0.7 entropy-outcome correlation target (actual r≈−0.09); reframed from 'entropy is optimal' to 'empirical improvement despite weak signal'.\",\n    \"Narrowed scope to synthetic-only validation; removed claim of validation 'on both synthetic and real workloads.'\",\n    \"Added mandatory real-world validation requirement (Alibaba 2018 traces) with lower success bounds (≥15% CV, ≥8% p95) before production deployment claims.\",\n    \"Increased sample size requirement from n=10 to n≥50 trials per scheduler for statistical rigor; added power analysis.\",\n    \"Added mandatory baseline comparisons to published methods (MoS, PREACT, etc.) rather than optional future work.\",\n    \"Emphasized mechanism uncertainty: weak entropy-outcome correlation suggests non-direct signal relationship; alternative hypotheses (adaptive-thresholds-in-general, non-linear relationships) flagged for investigation.\",\n    \"Required empirical threshold justification (grid-search on synthetic data) rather than relying on candidate parameter values.\",\n    \"Softened parameter robustness claims pending larger sensitivity analysis and real-world validation.\",\n    \"Added entropy distribution divergence measurement (KS test) for synthetic vs. real comparison.\"\n  ],\n  \"relation_type\": \"evolution\",\n  \"relation_rationale\": \"Narrowed scope to synthetic; acknowledged r>0.7 failure; shifted to empirical basis.\",\n  \"artifact_relations\": [\n    {\n      \"from_id\": \"art_sTVyGK2NrtrJ\",\n      \"to_id\": \"art_aWntWiTqo8og\",\n      \"relation_type\": \"uses\",\n      \"relation_rationale\": \"Experiment implements entropy-adaptive design spec (entropy estimator, momentum tracker, threshold formula).\"\n    }\n  ]\n}\n", "structuredPatch": [{"oldStart": 54, "oldLines": 7, "newStart": 54, "newLines": 7, "lines": ["     \"Added entropy distribution divergence measurement (KS test) for synthetic vs. real comparison.\"", "   ],", "   \"relation_type\": \"evolution\",", "-  \"relation_rationale\": \"Same core concept (entropy+momentum adaptive scheduling), but narrowed to synthetic-only validation, acknowledged r>0.7 target failure, and shifted from theoretical optimality to empirical pragmatism.\",", "+  \"relation_rationale\": \"Narrowed scope to synthetic; acknowledged r>0.7 failure; shifted to empirical basis.\",", "   \"artifact_relations\": [", "     {", "       \"from_id\": \"art_sTVyGK2NrtrJ\","]}], "originalFile": "{\n  \"title\": \"Entropy+Momentum Adaptive Scheduling (Synthetic Validation)\",\n  \"hypothesis\": \"Background job scheduling improves under bursty workloads when thresholds adapt dynamically based on CPU load entropy (unpredictability) and momentum (rate of change), rather than using fixed absolute thresholds. Empirical validation on synthetic workloads achieves: (1) background job completion time variance reduced by ≥20% vs. fixed-55% threshold (measured 23.5%, p<0.001), (2) foreground p95 latency improved by ≥10% (measured 11.4%, p<0.001), (3) both entropy and momentum components demonstrably contribute (Cohen's d > 1.0 for each), (4) design parameters near-optimal within tested ranges, (5) superiority confirmed vs. momentum-only and entropy-only baselines. CRITICAL LIMITATION: Validation currently limited to synthetic Poisson workloads with periodic spikes; real production workloads exhibit different entropy distributions (0.4-1.0 bits vs. 0.2-0.6 bits synthetic) and temporal structure (multi-scale burstiness, diurnal patterns), requiring empirical re-validation. Entropy-outcome correlation is weak (r≈-0.09, target r>0.7 not achieved), suggesting entropy works empirically but mechanism is not direct signal correlation; alternative explanations (adaptive-thresholds-in-general, non-linear relationships) require investigation.\",\n  \"motivation\": \"Current fixed CPU threshold schedulers fail under bursty loads because they cannot distinguish safe calm periods from temporary lulls before spikes. Entropy quantifies load unpredictability and momentum captures trends, enabling dynamic dispatch decisions. However, the weak entropy-outcome correlation (r≈-0.09) indicates entropy's theoretical optimality is unconfirmed; the contribution is empirical robustness of adaptive thresholds, not theoretically optimal signal selection.\",\n  \"assumptions\": [\n    \"CPU load entropy from 60s windows provides useful scheduling signal, even if not strongly correlated with instantaneous outcomes.\",\n    \"Load momentum is useful for dispatch timing refinement.\",\n    \"Background jobs tolerate adaptive scheduling windows.\",\n    \"Entropy-based adaptation reduces variance vs. fixed thresholds on synthetic bursty workloads.\",\n    \"Real production workloads require entropy re-calibration due to different distributions than synthetic models.\"\n  ],\n  \"investigation_approach\": \"Expand and validate synthetic work: (1) Enlarge to n≥50 trials per scheduler to ensure statistical adequacy (vs. current n=10); report power analysis and stable confidence intervals. (2) Investigate entropy-outcome correlation failure: plot scatter entropy vs. latency delta; fit polynomial and non-linear models; test alternative signals (load variance, autocorrelation, ARIMA error); compare correlation strength across signals. (3) Add real workload validation: extract CPU load from Alibaba 2018 traces (primary dataset); run entropy-adaptive and baseline; measure CV and p95; compare entropy distributions via KS test; measure whether parameters need retuning. (4) Baseline comparisons: implement momentum-only (fixed 55% + momentum dispatch) and at least one published baseline (e.g., MoS workload-type classifier); report head-to-head comparison. (5) Empirically justify thresholds: grid-search momentum threshold, entropy safe-zone, and false-positive window on synthetic data; report sensitivity.\",\n  \"success_criteria\": \"On synthetic workloads (n≥50): (a) ≥20% CV reduction vs. fixed-55% (p<0.05), (b) ≥10% p95 improvement (p<0.05), (c) entropy and momentum each Cohen's d>1.0 in ablation, (d) superiority over momentum-only and ≥1 published baseline on identical workload, (e) parameter choices justified via grid-search sensitivity analysis. Real workload validation (Alibaba 2018): (f) ≥15% CV reduction on real traces (lower bound accounting for entropy mismatch), (g) ≥8% p95 improvement on real traces, (h) entropy distribution divergence documented (KS test p-value and Cohen's d), (i) parameter recalibration strategy identified if needed.\",\n  \"related_works\": [\n    \"Mixture-of-Schedulers (2511.11628): Routes via workload-type ML classification. Differs: entropy-adaptive uses information theory for threshold adaptation, not workload classification; requires no training data.\",\n    \"PREACT (3673038.3673135): QPS prediction at datacenter scale. Differs: entropy-adaptive operates single-machine, dispatch-time (not provision-time).\",\n    \"DriftSched (2606.02982): GPU token drift adaptation. Differs: entropy-adaptive uses universally available CPU load, not GPU-specific signals.\",\n    \"Load trend analysis: Uses forecasting for provisioning. Differs: entropy-adaptive uses current entropy+momentum for real-time dispatch decisions.\",\n    \"RL entropy scheduling (2601.19624): Exploration policy tuning. Differs: context is learning, not CPU job scheduling.\"\n  ],\n  \"terms\": [\n    {\n      \"term\": \"Load entropy\",\n      \"definition\": \"Shannon entropy of recent CPU load samples (60s window, 8 bins), quantifying unpredictability. High entropy = volatile, risky to schedule; low entropy = smooth, safe to schedule.\"\n    },\n    {\n      \"term\": \"Momentum (load gradient)\",\n      \"definition\": \"Instantaneous rate of change of CPU load (estimated via EWMA, α=0.2). Negative momentum = decreasing load (safe dispatch window); positive momentum = increasing load (risky dispatch).\"\n    },\n    {\n      \"term\": \"Adaptive threshold\",\n      \"definition\": \"CPU load limit that scales based on entropy (high entropy → conservative, low entropy → permissive), e.g., 70% - 45%×H_norm.\"\n    },\n    {\n      \"term\": \"Bursty workload\",\n      \"definition\": \"High-variability request rate causing unpredictable CPU spikes even with moderate mean load.\"\n    },\n    {\n      \"term\": \"Background job\",\n      \"definition\": \"Lower-priority task (GC, indexing, replication) tolerating variable scheduling delays without degrading foreground latency.\"\n    }\n  ],\n  \"summary\": \"Entropy-adaptive background job scheduling improves job completion time variance and foreground latency on synthetic bursty workloads by adjusting CPU thresholds based on entropy and momentum. Synthetic validation shows 23.5% variance reduction and 11.4% latency improvement. Scope is currently synthetic-only; real production workload validation (Alibaba 2018) is required before production claims. Weak entropy-outcome correlation (r≈-0.09) indicates mechanism is not theoretically grounded; alternative hypotheses and non-linear relationships require investigation.\",\n  \"confidence_delta\": \"decreased\",\n  \"key_changes\": [\n    \"Acknowledged explicit failure of r>0.7 entropy-outcome correlation target (actual r≈−0.09); reframed from 'entropy is optimal' to 'empirical improvement despite weak signal'.\",\n    \"Narrowed scope to synthetic-only validation; removed claim of validation 'on both synthetic and real workloads.'\",\n    \"Added mandatory real-world validation requirement (Alibaba 2018 traces) with lower success bounds (≥15% CV, ≥8% p95) before production deployment claims.\",\n    \"Increased sample size requirement from n=10 to n≥50 trials per scheduler for statistical rigor; added power analysis.\",\n    \"Added mandatory baseline comparisons to published methods (MoS, PREACT, etc.) rather than optional future work.\",\n    \"Emphasized mechanism uncertainty: weak entropy-outcome correlation suggests non-direct signal relationship; alternative hypotheses (adaptive-thresholds-in-general, non-linear relationships) flagged for investigation.\",\n    \"Required empirical threshold justification (grid-search on synthetic data) rather than relying on candidate parameter values.\",\n    \"Softened parameter robustness claims pending larger sensitivity analysis and real-world validation.\",\n    \"Added entropy distribution divergence measurement (KS test) for synthetic vs. real comparison.\"\n  ],\n  \"relation_type\": \"evolution\",\n  \"relation_rationale\": \"Same core concept (entropy+momentum adaptive scheduling), but narrowed to synthetic-only validation, acknowledged r>0.7 target failure, and shifted from theoretical optimality to empirical pragmatism.\",\n  \"artifact_relations\": [\n    {\n      \"from_id\": \"art_sTVyGK2NrtrJ\",\n      \"to_id\": \"art_aWntWiTqo8og\",\n      \"relation_type\": \"uses\",\n      \"relation_rationale\": \"Experiment implements entropy-adaptive design spec (entropy estimator, momentum tracker, threshold formula).\"\n    }\n  ]\n}\n", "userModified": false}
```

### [11] ASSISTANT · 2026-08-24 11:48:28 UTC

```
Done. Fixed the `relation_rationale` field to 82 characters: "Narrowed scope to synthetic; acknowledged r>0.7 failure; shifted to empirical basis."

The revised hypothesis JSON now passes schema validation and is ready for the next iteration.
```
